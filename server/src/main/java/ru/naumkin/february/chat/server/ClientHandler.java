...
public class ClientHandler {
    ...
    private void tryToKick(String message) {
        ...
        String roleOfTheUserBeingKicked = server.getUserService().getUserRole(userNameForKick);
        String roleOfTheRequesterKick = server.getUserService().getUserRole(userName);

        if (!roleOfTheRequesterKick.equals("admin")) {
            sendMessage("СЕРВЕР: вы не обладаете правами ADMIN для кика пользователя из чата");
            return;
        }

        if (roleOfTheUserBeingKicked.equals("admin")) {
